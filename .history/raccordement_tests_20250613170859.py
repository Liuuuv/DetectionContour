...
alpha = 2
for j in range(len(column_list_fin)):
    if j >= 1 and j < len(column_list_fin) - 1:
        
        frac = intensity_list - np.floor(intensity_list)
        to_add = intensity_list[j-1][0] * (1 - ease_in_out(frac, alpha)) + intensity_list[j+1][0] * ease_in_out(frac, alpha)
        interpol_intensity_list.append(to_add)
    else:
        interpol_intensity_list.append(intensity_list[j][0])
