...
def edge_detection_2(img):
    ...
    magnitude = get_magnitude(image_x, image_y)
...
def get_difference(img_):
    ...
    for i in range(len(img_)-1):
        for j in range(len(img_[i])-1):
            if (np.abs(img_[i+1,j]) - np.abs(img_[i,j]) != 0).all() or (np.abs(img_[i+1,j+1]) - np.abs(img_[i,j]) != 0).all() or (np.abs(img_[i,j+1]) - np.abs(img_[i,j]) != 0).all():
                img[i,j] = [1, 1, 1]
            else:
                img[i,j] = [0, 0, 0]
    return img
