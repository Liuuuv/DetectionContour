...
widths = np.arange(0, image.shape[1])
cwtmatr, freqs = pywt.cwt(image[50,:,0])

surface = np.sum(np.abs(cwtmatr), widths, 'morlet')

# Affichage
plt.imshow(np.abs(cwtmatr), extent=[0, image.shape[1], 1, 31], cmap='jet')
