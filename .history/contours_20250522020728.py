...
#     return np.stack((convolved,)*3, axis=-1)[:,:]*255
...
def extrem_threshold(img, threshold):
    return np.where(img > threshold, 255, np.where(img < -threshold, -255, 0))
...
def edge_detection_2(img):
    image_x = convolution(img, filterx)[:,:,0]
    image_y = convolution(img, filtery)[:,:,0]
    image_xy = convolution(img, filterxy)[:,:,0]
    image_xx = convolution(img, filterxx)[:,:,0]
    image_yy = convolution(img, filteryy)[:,:,0]
    ...
    # img[:,:] *= 255
    # return  img_
    return np.stack((img_,)*3, axis=-1).astype(np.uint8)
...
def get_difference(img_):
    ...
    for i in range(len(img_)-1):
        for j in range(len(img_[i])-1):
            if (np.abs(img_[i+1,j]) - np.abs(img[i,j]) != 0).all() or (np.abs(img_[i+1,j+1]) - np.abs(img[i,j]) != 0).all() or (np.abs(img_[i,j+1]) - np.abs(img[i,j]) != 0).all():
                img[i,j] = [255, 255, 255]
            else:
                img[i,j] = [0, 0, 0]
    return img
...
image1 = extrem_threshold(image1, 60)
# image1 = get_sign_color(image1)
image1 = get_difference(image1)

# # Détection des positions de changement
# changes = np.diff(np.sign(image1)) != 0
# print(changes.shape, image1.shape)
# changes = np.pad(changes, ((0, 0), (0, 0), (0, 1)), mode='constant')
# print(changes.shape, image1.shape)


# image1 = np.where(changes, 255,0)



# image1 = np.zeros_like(image1, dtype=np.uint8)
# image1[:][changes] = 255  # Pixel après le changement
# image1[:][changes] = 255 # Pixel avant le changement

plot(image1)

# image2 = edge_detection_1(image)
# image2 = threshold(image2, 5)
# plot(image2)
