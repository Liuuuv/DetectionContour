...
cH = np.stack((cA,)*3, axis=-1).astype(np.float32)
cV = np.stack((cA,)*3, axis=-1).astype(np.float32)
cD = np.stack((cA,)*3, axis=-1).astype(np.float32)
print(cH.shape)
# cA = np.concatenate([cA, cA[:, :, -1:]], axis=2)
# cA = np.uint8(cA)
# print(cA.shape)
# print(image.shape)
plt.imshow(cH)
