...
def print_aligned_mixed_matrix(matrix):
    # Convertir tous les éléments en strings avec un formatage aligné
    max_len = max(len(str(element)) for row in matrix for element in row)
    
    for row in matrix:
        # Formater chaque élément avec un padding uniforme
        formatted_row = []
        for element in row:
            if isinstance(element, tuple):
                # Pour les tuples : (val1, val2)
                s = f"({str(element[0]).rjust(2)}, {str(element[1]).ljust(2)})"
            else:
                # Pour les entiers
                s = str(element).center(max_len)
...
print_aligned_mixed_matrix(convolution_matrix)