...
def get_magnitude(img_x, img_y, show_angle = False):
    ...
    if not show_angle:
        ...
        # Calcul de l'angle (avec arctan2 pour couvrir tout le cercle)
        angles = np.arctan2(img_y, img_x)  # [-π, π]
        angles = (angles + np.pi) % (2*np.pi)  # [0, 2π]
        
        # Normalisation HSV
        h = angles / (2*np.pi)  # Teinte [0,1]
        ...
        # Combinaison magnitude + couleur
        return (rgb * magnitude[..., np.newaxis]).astype(np.uint8)
