...
def convolution(img_: np.ndarray, filter: 'Filter') -> np.ndarray:
    ...
    fh, fw = filter_arr.shape[:2]  # Filter height and width
    
    # Calculate padding needs
    pad_top = fc_x
    pad_bottom = fh - fc_x - 1
    pad_left = fc_y
    pad_right = fw - fc_y - 1
    
    # Pad the image
    padded_img = np.pad(img_, ((pad_top, pad_bottom), (pad_left, pad_right)), 
    ...
    for i in range(h):
        for j in range(w):
            ...
            patch = padded_img[i:i+fh, j:j+fw]
            
            # Compute convolution
            if img_.ndim == 3:  # Color image
                for c in range(img_.shape[2]):
                    img[i,j,c] = abs(np.sum(patch[..., c] * filter_arr))
            else:  # Grayscale
                img[i,j] = abs(np.sum(patch * filter_arr))
