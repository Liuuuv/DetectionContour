...
for i, (cH, cV, cD) in enumerate(details):
    if i == 0:
        ...
        threshold_ = 1/(i+1)
        cH = threshold_too_much(cH, threshold_)
        cV = threshold_too_much(cV, threshold_)
        cD = threshold_too_much(cD, threshold_)
...
def get_cwt_compatible_wavelets():
    """Retourne la liste des ondelettes compatibles avec CWT dans PyWavelets"""
    wavelets = pywt.wavelist()
    cwt_wavelets = []
    
    for wavelet in wavelets:
        try:
            # Teste si l'ondelette a une fonction pour CWT
            if hasattr(pywt.ContinuousWavelet(wavelet), 'complex_cwt'):
...
print(get_cwt_compatible_wavelets())
