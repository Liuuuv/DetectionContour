...
def get_difference(img_):
    ...
    for i in range(len(img_)-1):
        for j in range(len(img_[i])-1):
            if np.abs(img_[i+1,j]) - np.abs(img[i,j]) == np.array([0,0,0]) or np.abs(img_[i+1,j+1]) - np.abs(img[i,j]) == [0,0,0] or np.abs(img_[i,j+1]) - np.abs(img[i,j]) == [0,0,0]:
...
image1 = extrem_threshold(image1, 15)
