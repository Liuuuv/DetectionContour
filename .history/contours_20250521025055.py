...
def convolution(img_: np.ndarray, filter: 'Filter') -> np.ndarray:
    """
    Convolution 2D corrigée avec gestion des bords et multi-canaux
    
    Args:
        img_: Image d'entrée (2D ou 3D)
        filter: Objet avec attributs:
    ...
    pad_h = (k_h - 1) // 2
    pad_w = (k_w - 1) // 2
    padded = np.pad(img_, ((pad_h, pad_h), (pad_w, pad_w)), mode='constant')
    
    # Convolution
    for i in range(h):
        for j in range(w):
            # Extraction région d'intérêt
            region = padded[i:i+k_h, j:j+k_w]
            
            # Application différente selon les canaux
            if img_.ndim == 3:
                for c in range(img_.shape[2]):
                    img[i,j,c] = np.sum(region[..., c] * kernel)
            else:
                img[i,j] = np.sum(region * kernel)
