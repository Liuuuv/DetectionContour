...
studied_row = 25
widths = np.arange(1, 31)  # Échelles analysées
cwtmatr, freqs = pywt.cwt(image[studied_row,:,0], widths, 'morl')  # Ondelette de Morlet
...
plot(np.abs(cwtmatr), extent=[0, image.shape[1], 1, 31], cmap='jet')
plt.colorbar()
...
if multiplot:
    plt.subplots_adjust(
        left=0.05, right=1, bottom=0, top=1,
        wspace=0, hspace=0
    )
