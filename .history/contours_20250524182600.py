...
def edge_detection_wt(img, level):
    ...
    edges = np.zeros((img.shape[0]//2+1, img.shape[1]//2), dtype=np.float32)
    for cH, cV, cD in details:
        print(cH.shape, cV.shape, cD.shape, edges.shape)
        edges += np.sqrt(cH**2 + cV**2 + cD**2)
    
    edges = np.stack((edges,)*3, axis=-1).astype(np.float32)
    return edges
