...
def find_best_threshold(img, tolerance = .00005):
    step = .001
    current_threshold = 0
...
def plot_noise_score(img, st_deviation_list):
    ...
    for st_deviation in st_deviation_list:
        ...
        threshold, score = find_best_threshold(noised_image)
        score_list.append(score)
    
    plt.plot(st_deviation, score_list)
    plt.grid()
...
st_deviation_list = np.linspace(0,0.5,50)
