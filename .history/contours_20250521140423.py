...
def get_magnitude(img_x, img_y, show_angle = False):
    if not show_angle:
        ...
        angles = np.arctan(img_y/img_x)[:,:,1]
        # normalized = (angles % (2*np.pi)) / (2*np.pi)
    
        # # Trois phases décalées pour R, G, B (décalage de 2π/3)
        # r = (np.sin(2*np.pi * normalized + 0) * 0.5 + 0.5)
        # g = (np.sin(2*np.pi * normalized + 2*np.pi/3) * 0.5 + 0.5)
        # b = (np.sin(2*np.pi * normalized + 4*np.pi/3) * 0.5 + 0.5)
        
        # # Combinaison et conversion en uint8
...
image1 = edge_detection_1(image, False)
