...
def optimized_convolution(img_: np.ndarray, filter: 'Filter') -> np.ndarray:
    """
    Convolution optimisée pour images noir et blanc ou RGB (canaux identiques)
    
    Args:
        img_: Image d'entrée (HxW ou HxWx3)
        filter: Objet avec:
            array: Noyau de convolution 2D
            center: Position centrale du filtre
            
    Returns:
        Image convoluée (même format que l'entrée)
    """
    ...
    if img_.ndim == 3:
        return np.stack((convolved,)*3, axis=-1).astype(img_.dtype)
    return convolved.astype(img_.dtype)
...
image1 = get_difference(image1, .05)
# plt.imshow(image1)
